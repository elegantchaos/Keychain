...
final class KeychainTests: XCTestCase {
  static let exampleCreator: UInt32 = 0x7554_7374 /* corresponds to 'uTst' */
  ...
  private func makeExecutableTestKeychain() -> Keychain {
    ...
    func testAccessGroupIsAddedWhenProvided() throws {
      let keychain = Keychain(storage: .dataProtection, accessGroup: "group.com.elegantchaos.actionstatus")

      let spec = keychain.itemSpec(for: "unittest", on: "server")

      XCTAssertEqual(spec[kSecAttrAccessGroup] as? String, "group.com.elegantchaos.actionstatus")
    }

    func testGenericPasswordUsesServiceAttribute() throws {
      let keychain = Keychain(kind: kSecClassGenericPassword, storage: .dataProtection)

      let spec = keychain.itemSpec(for: "unittest", on: "service")

      XCTAssertEqual(spec[kSecAttrService] as? String, "service")
      XCTAssertNil(spec[kSecAttrServer])
    }
